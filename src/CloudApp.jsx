...
function AccessForm({ recovery = false, onRecovered }) {
  ...
  const submit = async event => {
    ...
    try {
      ...
      const result = recovery ? await supabase.auth.updateUser({ password })
        : mode === 'signup' ? await supabase.auth.signUp({ email: email.trim(), password, options: { emailRedirectTo: redirectTo } })
          : mode === 'reset' ? await supabase.auth.resetPasswordForEmail(email.trim(), { redirectTo })
            : await supabase.auth.signInWithPassword({ email: email.trim(), password })
      if (result.error) throw result.error
      if (recovery) onRecovered()
      else if (mode === 'signup') setMessage('Revisa tu correo para confirmar la cuenta y después inicia sesión.')
  ...
  return <main className="access-page"><section className="surface access-card"><div className="brand-mark">IS</div><h1 className="page-title mt-5">InkStudio</h1><p className="mt-3 mb-6 text-slate-400">{recovery ? 'Elige una nueva contraseña.' : mode === 'signup' ? 'Crea tu cuenta para administrar un espacio privado del estudio.' : mode === 'reset' ? 'Recupera el acceso a tu cuenta.' : 'Inicia sesión para acceder a los datos de tu estudio.'}</p><form className="form-fields" onSubmit={submit}>{error && <Alert severity="error">{error}</Alert>}{message && <Alert severity="success">{message}</Alert>}{!recovery && <TextField label="Correo electrónico" type="email" autoComplete="email" required value={email} onChange={event => setEmail(event.target.value)} />}{(recovery || mode !== 'reset') && <TextField label={recovery ? 'Nueva contraseña' : 'Contraseña'} type="password" autoComplete={recovery || mode === 'signup' ? 'new-password' : 'current-password'} required value={password} onChange={event => setPassword(event.target.value)} slotProps={{ htmlInput: { minLength: recovery || mode === 'signup' ? 8 : 1 } }} helperText={recovery || mode === 'signup' ? 'Usa al menos 8 caracteres.' : ''} />}<Button type="submit" variant="contained" disabled={busy}>{busy ? 'Procesando…' : recovery ? 'Guardar contraseña' : mode === 'signup' ? 'Crear cuenta' : mode === 'reset' ? 'Enviar enlace' : 'Iniciar sesión'}</Button>{!recovery && <><Button disabled={busy} onClick={() => { setMode(mode === 'signup' ? 'login' : 'signup'); setMessage(''); setError('') }}>{mode === 'signup' ? 'Ya tengo cuenta' : 'Crear una cuenta'}</Button><Button disabled={busy} onClick={() => { setMode(mode === 'reset' ? 'login' : 'reset'); setMessage(''); setError('') }}>{mode === 'reset' ? 'Volver al inicio de sesión' : 'Olvidé mi contraseña'}</Button></>}</form></section></main>
}
